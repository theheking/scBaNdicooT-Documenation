...
This project introduces a nascent single-cell RNA sequencing framework designed to overcome that challenge. By combining metabolic RNA labelling with single-cell sequencing, we directly measure newly synthesised RNA at single-cell resolution. This reveals how cells dynamically respond to stress, drugs, and environmental cues, rather than relying on steady-state gene expression alone.

In the paper, we apply this approach in a physiologically relevant 3D tumouroid model, preserving tumour architecture and the native tumour microenvironment. Using ex vivo tumoroids derived from a genetically engineered mouse model of breast cancer, we profile transcriptional responses to chemotherapy across thousands of individual cells, spanning epithelial, immune and endothelial populations.
...
This resource is intended for researchers who want to investigate dynamic gene regulation in heterogeneous cell populations by applying nascent RNA sequencing to heterogeneous tissues. 

Specifically, it provides bioinformatics steps for analysing nascent single-cell RNA-seq data generated using:
